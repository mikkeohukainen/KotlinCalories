...
fun CalorieScreen() {
    ...
    Column (
        ...
        IntensityList(myOnClick = { intensity = it })
        Text(text = result.toString(), color = MaterialTheme.colorScheme.secondary, fontWeight = FontWeight.Bold)
        Calculation(male = male, weight = weightInput.toIntOrNull() ?: 0, intensity = intensity, setResult = { result = it })
    }
}
...
@Composable
fun IntensityList(myOnClick: (Float) -> Unit) {
